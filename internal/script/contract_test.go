...
var userOnlyRoutes = []omittedRoute{
	...
	{route{http.MethodPost, "/v1/scripts/{}:run"}, userCredentialRequired},
...
func TestContractParity(t *testing.T) {
	...
	if len(cli) != len(cliRoutes) || len(cli) != 25 {
		t.Fatalf("cli routes: got %d unique of %d, want 25", len(cli), len(cliRoutes))
	}
	...
	if len(docsMinusCLI) != 3 {
		t.Fatalf("docs-minus-CLI: got %d, want 3\n%s", len(docsMinusCLI), formatRoutes(docsMinusCLI))
	}
