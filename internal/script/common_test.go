...
const (
	testKey    = "sl_key_test_fixture"
	testAccess = "oauth-access-token-fixture"
)
...
func testTransport(t *testing.T, srv *httptest.Server) *http.Client {
	t.Helper()
	u, err := url.Parse(srv.URL)
	if err != nil {
		t.Fatal(err)
	}
	return &http.Client{Transport: hostRewrite{target: u, next: http.DefaultTransport}}
}

func testClient(t *testing.T, srv *httptest.Server, namespace string) *apiclient.Client {
	t.Helper()
	return apiclient.New(apiclient.Options{APIKey: testKey, NamespaceID: namespace, APIDomain: "example.test", UserAgent: "sl/test", HTTPClient: testTransport(t, srv)})
}

func testOAuthClient(t *testing.T, srv *httptest.Server, namespace string) *apiclient.Client {
	t.Helper()
	return apiclient.New(apiclient.Options{AccessToken: testAccess, NamespaceID: namespace, APIDomain: "example.test", UserAgent: "sl/test", HTTPClient: testTransport(t, srv)})
}
...
func execScript(t *testing.T, client *apiclient.Client, stdin io.Reader, stdinTTY bool, confirm func(string) error, args ...string) execResult {
	t.Helper()
	return execScriptOAuth(t, client, stdin, stdinTTY, confirm, func() error { return nil }, args...)
}

func execScriptOAuth(t *testing.T, client *apiclient.Client, stdin io.Reader, stdinTTY bool, confirm func(string) error, oauth func() error, args ...string) execResult {
	t.Helper()
...
func assertScriptOAuthHeaders(t *testing.T, h http.Header, namespace string) {
	t.Helper()
	if h.Get("Authorization") != "Bearer "+testAccess {
		t.Fatalf("Authorization=%q", h.Get("Authorization"))
	}
	if h.Get("sl-api-key") != "" {
		t.Fatalf("sl-api-key present")
	}
	if h.Get("sl-namespace-id") != namespace {
		t.Fatalf("sl-namespace-id=%q", h.Get("sl-namespace-id"))
	}
...
func assertNoTokens(t *testing.T, stdout, stderr string) {
	t.Helper()
	combined := stdout + stderr
	for _, needle := range []string{testKey, testAccess, "Bearer ", "access_token", "refresh_token"} {
		if strings.Contains(combined, needle) {
			t.Fatalf("output leaked %q", needle)
		}
	}
}
