...
func newVersionGetCommand(f *cli.Factory, workflowID *string) *cobra.Command {
	cmd := &cobra.Command{Use: "get VERSION --workflow ID", Short: "Get a workflow version", SilenceUsage: true, SilenceErrors: true, Args: cobra.ExactArgs(1)}
...
func newVersionRevertCommand(f *cli.Factory, workflowID *string) *cobra.Command {
	cmd := &cobra.Command{Use: "revert VERSION --workflow ID", Short: "Revert a workflow version", SilenceUsage: true, SilenceErrors: true, Args: cobra.ExactArgs(1)}
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
