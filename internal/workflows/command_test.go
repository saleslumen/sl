...
func TestWorkflowsCommandTree(t *testing.T) {
	...
	want := []string{
		...
		"sl workflows executions resume",
		"sl workflows executions start",
		"sl workflows get",
		"sl workflows history",
		"sl workflows list",
		"sl workflows publish",
		"sl workflows triggers",
		"sl workflows triggers create",
...
func TestNoExcludedCommands(t *testing.T) {
	...
	walk = func(c *cobra.Command) {
		...
		if _, ok := forbidden[c.CommandPath()]; ok {
...
func TestRequiredFlagsAppearInUse(t *testing.T) {
	...
	cases := []struct {
		...
		{path: []string{"triggers", "create"}, use: "create --workflow ID --input FILE"},
		...
		{path: []string{"executions", "start"}, use: "start --workflow ID --input FILE"},
		{path: []string{"executions", "resume"}, use: "resume EXECUTION_ID --workflow ID --input FILE"},
...
func TestHelpStatesUserOnlyNotices(t *testing.T) {
	cases := []struct {
		...
		{args: []string{"workflows", "--help"}, want: []string{"user OAuth", "sl auth login", "anonymous"}},
		{args: []string{"workflows", "executions", "--help"}, want: []string{"user OAuth", "sl auth login"}},
		{args: []string{"workflows", "triggers", "--help"}, want: []string{"user OAuth", "sl auth login"}},
	}
	for _, tc := range cases {
