...
type Workflow struct {
	...
	DraftVersion   *WorkflowVersion `json:"draftVersion,omitempty"`
}
...
func draftVersionNumber(row Workflow) string {
	if row.DraftVersion == nil {
		return ""
	}
	return formatUint32(row.DraftVersion.Version)
}
...
func workflowColumns() []output.Column[Workflow] {
	return []output.Column[Workflow]{
		...
		{Header: "DRAFT", Value: draftVersionNumber},
...
func versionColumns() []output.Column[WorkflowVersion] {
	return []output.Column[WorkflowVersion]{
		...
		{Header: "DRAFT", Value: func(row WorkflowVersion) string { return strconv.FormatBool(row.IsDraft) }},
