...
func TestTriggersCreateSendsBodyAndNamespace(t *testing.T) {
	raw := `{"trigger":{"id":"trg-9","kind":1,"enabled":true,"disabledReason":0,"updatedAt":"2026-01-02T03:04:05Z"}}`
	inputBody := `{"kind":1,"enabled":true,"webhook":{"path":"/lead"}}`
	var requests []capturedRequest
	f, out, stderr := testFactory(t, testOAuthClient(t, captureServer(t, http.StatusOK, raw, &requests), "ns-1"), factoryOptions{})
	if err := execute(t, f, "triggers", "create", "--workflow", "wf-1", "--input", writeInput(t, inputBody)); err != nil {
		t.Fatalf("execute: %v", err)
	}
	if len(requests) != 1 {
		t.Fatalf("requests=%d", len(requests))
	...
	req := requests[0]
	if req.method != http.MethodPost || req.path != "/v1/workflows/wf-1/triggers" || req.query != "" {
		t.Fatalf("request %s %s?%s", req.method, req.path, req.query)
	}
	if strings.Contains(req.body, `"trigger"`) {
		t.Fatalf("wrapped body=%s", req.body)
	}
	assertJSONEqual(t, req.body, inputBody)
	assertApplicationHeaders(t, req.headers, map[string]string{"Authorization": "Bearer " + testAccess, "Sl-Namespace-Id": "ns-1", "User-Agent": "sl/test", "Accept": "application/json", "Content-Type": "application/json"})
	if out.String() != "trg-9\tWEBHOOK\ttrue\tUNSPECIFIED\t2026-01-02T03:04:05Z\n" {
		t.Fatalf("stdout=%q", out.String())
	}
	assertNoTokens(t, out.String(), stderr.String())
}

func TestTriggersCreateRequiresInput(t *testing.T) {
	var requests []capturedRequest
	f, out, stderr := testFactory(t, testClient(t, captureServer(t, http.StatusOK, `{}`, &requests), ""), factoryOptions{})
	err := execute(t, f, "triggers", "create", "--workflow", "wf-1")
	if usageError(t, err) != "--input is required" {
		t.Fatalf("err=%v", err)
	}
	if len(requests) != 0 {
...
func TestTriggersCreateRequiresUserOAuth(t *testing.T) {
	var requests []capturedRequest
	f, out, stderr := testFactory(t, testClient(t, captureServer(t, http.StatusOK, `{}`, &requests), ""), factoryOptions{})
	f.RequireUserOAuth = func() error { return cli.ErrUserOAuthRequired }
	err := execute(t, f, "triggers", "create", "--workflow", "wf-1", "--input", writeInput(t, `{"kind":1}`))
