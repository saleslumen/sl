...
func newExecutionsCommand(f *cli.Factory) *cobra.Command {
	...
	cmd.AddCommand(newExecutionsStartCommand(f, &workflowID))
	cmd.AddCommand(newExecutionsResumeCommand(f, &workflowID))
...
func newExecutionsStartCommand(f *cli.Factory, workflowID *string) *cobra.Command {
	var source string
	cmd := &cobra.Command{Use: "start --workflow ID --input FILE", Short: "Start a workflow execution", SilenceUsage: true, SilenceErrors: true, Args: cobra.NoArgs}
	cmd.Flags().StringVar(&source, "input", "", "JSON request body file, or - for stdin")
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		return runExecutionStart(cmd.Context(), f, *workflowID, source)
	}
	return cmd
}

func newExecutionsResumeCommand(f *cli.Factory, workflowID *string) *cobra.Command {
	var source string
	cmd := &cobra.Command{Use: "resume EXECUTION_ID --workflow ID --input FILE", Short: "Resume a paused workflow execution", SilenceUsage: true, SilenceErrors: true, Args: cobra.ExactArgs(1)}
	cmd.Flags().StringVar(&source, "input", "", "JSON request body file, or - for stdin")
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		return runExecutionResume(cmd.Context(), f, *workflowID, args[0], source)
	}
	return cmd
}
...
func runExecutionStart(ctx context.Context, f *cli.Factory, workflowID, source string) error {
	...
	resp, err := doRequest(ctx, f, http.MethodPost, workflowSubpath(workflowID, "executions")+":start", nil, body)
	if err != nil {
		return err
	}
	return printExecution(f, resp.Body)
...
func runExecutionResume(ctx context.Context, f *cli.Factory, workflowID, executionID, source string) error {
	if err := f.RequireUserOAuth(); err != nil {
		return err
	}
	path, err := executionResourcePath(workflowID, executionID)
	...
	resp, err := doRequest(ctx, f, http.MethodPost, path+":resume", nil, body)
	if err != nil {
		return err
	}
	return printExecution(f, resp.Body)
