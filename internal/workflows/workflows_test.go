...
func TestGetSendsPathWithoutBody(t *testing.T) {
	var requests []capturedRequest
	raw := `{"workflow":{"id":"wf-1","name":"hello","version":2,"isActive":false,"updatedAt":"t2","draftVersion":{"version":3,"isDraft":true}}}`
	...
	if out.String() != "wf-1\thello\t2\t3\tfalse\tt2\n" {
		t.Fatalf("stdout=%q", out.String())
	}
}
...
func TestHistorySendsPathAndRendersVersions(t *testing.T) {
	var requests []capturedRequest
	raw := `{"versions":[{"version":2,"id":"ver-2","isLive":true,"isDraft":false,"createdAt":"t2"},{"version":3,"id":"ver-3","isLive":false,"isDraft":true,"createdAt":"t3"},{"version":1,"id":"ver-1","isLive":false,"isDraft":false,"createdAt":"t1"}]}`
	f, out, _ := testFactory(t, testClient(t, captureServer(t, http.StatusOK, raw, &requests), ""), factoryOptions{})
	...
	assertApplicationHeaders(t, got.headers, map[string]string{"Sl-Api-Key": testKey, "User-Agent": "sl/test", "Accept": "application/json"})
	if out.String() != "2\tver-2\ttrue\tfalse\tt2\n3\tver-3\tfalse\ttrue\tt3\n1\tver-1\tfalse\tfalse\tt1\n" {
		t.Fatalf("stdout=%q", out.String())
	}
}
...
func TestSharedColumns(t *testing.T) {
	if len(workflowColumns()) != 6 || len(versionColumns()) != 5 || len(triggerColumns()) != 5 || len(executionColumns()) != 6 || len(eventTypeColumns()) != 3 {
