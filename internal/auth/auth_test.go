...
func TestStatusPrintsPrefixNotKey(t *testing.T) {
	...
	cmd := NewCommand(Deps{
		...
		Resolve: func() (credentials.Resolved, error) {
			return credentials.Resolved{Profile: "work", APIKey: testKey, OrganizationID: "org-work", NamespaceID: "ns_1", APIDomain: "example.com"}, nil
		},
	})
	...
	for _, tc := range cases {
		t.Run(tc.product, func(t *testing.T) {
			...
			cmd := NewCommand(Deps{
				...
				Resolve: func() (credentials.Resolved, error) {
					return credentials.Resolved{Profile: "default", APIKey: testKey, APIDomain: credentials.DefaultAPIDomain}, nil
				},
