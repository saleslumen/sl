...
func newLoginCommand(d Deps) *cobra.Command {
	cmd := &cobra.Command{Use: "login", Short: "Log in to Saleslumen", SilenceUsage: true, SilenceErrors: true, Args: cobra.NoArgs}
	cmd.Flags().Bool("api-key", false, "Record an organization API key from a hidden prompt")
	cmd.Flags().Bool("with-token", false, "Read the API key from standard input")
	cmd.MarkFlagsMutuallyExclusive("api-key", "with-token")
...
func runLogin(cmd *cobra.Command, d Deps) error {
	...
	if !apiKey && !withToken {
		return runOAuthLogin(cmd.Context(), d)
	}
	return runAPIKeyLogin(d, withToken)
}

func runAPIKeyLogin(d Deps, withToken bool) error {
	...
	file.SetAPIKey(profile, key, organizationID(d))
...
func runStatus(cmd *cobra.Command, d Deps) error {
	...
	fmt.Fprintf(d.Streams.Out, "auth_mode: %s\n", resolved.AuthMode)
	...
	if resolved.AuthMode == credentials.AuthModeOAuth {
		fmt.Fprintf(d.Streams.Out, "oauth_user_id: %s\n", resolved.OAuthUserID)
	} else {
